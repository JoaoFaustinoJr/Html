...
self.addEventListener('fetch',event=>{
 ...
 if(event.request.mode==='navigate'){
  event.respondWith(fetch(event.request,{cache:'no-store'}).then(resp=>{if(resp&&resp.ok){const clone=resp.clone();caches.open(CACHE).then(c=>c.put('./index.html',clone)).catch(()=>{})}return resp}).catch(()=>caches.match('./index.html')));
  return;
 }
 const u=new URL(event.request.url);
 if(LIVE_RE.test(u.pathname)){
  event.respondWith(fetch(event.request,{cache:'no-store'}).then(resp=>{if(resp&&resp.ok){const clone=resp.clone();caches.open(CACHE).then(c=>c.put(event.request,clone)).catch(()=>{})}return resp}).catch(()=>caches.match(event.request,{ignoreSearch:true})));
  return;
 }
