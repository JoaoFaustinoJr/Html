(()=>{
 ...
 function wireX1Promo(){
  ...
  if(!localStorage.getItem('raiX1PilotLaunchSeenV1')&&!document.getElementById('raiX1Launch')){
   ...
   const close=()=>{localStorage.setItem('raiX1PilotLaunchSeenV1','1');m.classList.remove('show');setTimeout(()=>m.remove(),240)};
   m.querySelector('.go').addEventListener('click',()=>{localStorage.setItem('raiX1PilotLaunchSeenV1','1');x1Open();close()});m.querySelector('.later').addEventListener('click',close);m.querySelector('.rai-x1-launch-close').addEventListener('click',close);m.addEventListener('click',e=>{if(e.target===m)close()});
