(()=>{
 const VERSION='15.11.4',PATH='/Html/tangram-prof-junior-v12/';
 let reg=null,checking=null,reloading=false,specialStarted=false,aulasEntryStarted=false,iosInstallStarted=false,rewardsStarted=false,welcomeStarted=false,answerOrderStarted=false,verifyAssistStarted=false,lessonNarrationWired=false,lessonNarrationActive=false,lessonNarrationWatch=null;
 ...
 function sync(){syncHead();syncVersion();patchNarration();wireLessonNarration();loadVerifyAssist();loadAnswerOrder();loadAulasEntry();loadSpecial();loadRewards();loadWelcome();loadIOSInstall();try{window.__raiAnswerOrderV1?.scan?.();window.__raiEnsureAulasEntry?.();window.__raiProvaParanaV1?.ensureEntryPoints?.();window.__raiProvaRewardsV2?.refresh?.();window.__raiIOSInstall?.wire?.()}catch(e){}repairViewport()}
