(()=>{
 ...
 function wireX1Promo(){
  ...
  if(!localStorage.getItem('raiX1PilotLaunchSeenV1')&&!document.getElementById('raiX1Launch')){
   ...
   let autoClose=setTimeout(()=>close(false),5000);const close=mark=>{clearTimeout(autoClose);if(mark!==false)localStorage.setItem('raiX1PilotLaunchSeenV1','1');m.classList.remove('show');setTimeout(()=>m.remove(),240)};
