(()=>{
 ...
 function ensure(){
  ...
  overlay.innerHTML=`<div class="rai-pp-welcome-stamp" role="dialog" aria-modal="true" aria-labelledby="raiPPWelcomeTitle">
   ...
   <div class="rai-pp-welcome-badge">🎯 <span>ESPECIAL PROVA PARANÁ</span></div>
 ...
 function hide(){clearTimeout(timer);if(!overlay)return;overlay.classList.remove('show');setTimeout(()=>{if(overlay&&!overlay.classList.contains('show'))overlay.remove()},220)}
 function fill(kind='welcome',detail={}){
  ...
  const title=o.querySelector('.rai-pp-welcome-title'),text=o.querySelector('.rai-pp-welcome-text'),progress=o.querySelector('.rai-pp-welcome-progress'),go=o.querySelector('[data-go]');
  if(kind==='unlock'){
   title.textContent='🔓 Nova Missão Bônus!';
   ...
   title.textContent='Novas aulas + novas missões';
   ...
   title.textContent='🎯 Continue desbloqueando';
   ...
   title.textContent='🏆 Todas as missões liberadas';
 ...
 function show(force=false){
  ...
  fill();const o=ensure();requestAnimationFrame(()=>o.classList.add('show'));sessionStorage.setItem(SEEN,'1');clearTimeout(timer);timer=setTimeout(hide,5200);
 }
 function showUnlocked(detail){fill('unlock',detail||{});const o=ensure();requestAnimationFrame(()=>o.classList.add('show'));clearTimeout(timer);timer=setTimeout(hide,5200)}
