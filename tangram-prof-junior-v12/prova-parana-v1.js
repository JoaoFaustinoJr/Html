(()=>{
  if(window.__raiProvaParanaV1&&window.__raiProvaParanaV1.active)return;
  const runtime={active:true,mounted:false};
  window.__raiProvaParanaV1=runtime;
  let attempts=0;
  ...
  function init(){
    ...
    function ensureEntryPoints(){
      const currentDock=document.getElementById('raiLessonDock');
      if(currentDock){
        currentDock.classList.add('rai-pp-dock');
        if(!currentDock.querySelector('.rai-pp-dock-badge')){const badge=document.createElement('span');badge.className='rai-pp-dock-badge';badge.textContent='🎯 Paraná';currentDock.appendChild(badge)}
      }
      const currentHome=document.getElementById('raiLessonHome');
      if(currentHome&&!currentHome.querySelector('.rai-pp-hero'))currentHome.insertBefore(buildHero(),currentHome.firstChild);
    }
