...
if(!key){document.documentElement.classList.add('v53-ready');return;}
...
function activate(){
 ...
 selectMode();
 document.documentElement.classList.add('v53-ready','activity-ready');
 window.scrollTo({top:0,left:0,behavior:'auto'});
 return true;
}

function enforce(){if(!stopped)activate();}
[0,60,140,260,450,700,1050,1450].forEach(ms=>setTimeout(enforce,ms));
setTimeout(()=>{stopped=true;document.documentElement.classList.add('v53-ready');observer.disconnect();},1900);
...
document.addEventListener('click',e=>{
 if(e.target.closest('[data-home],#homeBrand,[data-nav="home"],.back-btn[data-home]')){
  e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();location.href='./?v=53';
 }
},true);

if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',()=>setTimeout(enforce,20),{once:true});
else enforce();
})();