...
function setExpression(exp,announce=true){if(!AVATARS[avatar]?.[exp])return;expression=exp;$$('[data-expression]').forEach(b=>b.classList.toggle('selected',b.dataset.expression===exp));refreshAvatar();if(announce)toast(feedback[exp]||'Que expressão especial!');halo()}
...
function preload(){Object.values(AVATARS).forEach(group=>Object.values(group).forEach(src=>{const i=new Image();i.decoding='async';i.src=src;}));}
function init(){
 ...
 $$('[data-mode]').forEach(b=>b.addEventListener('click',()=>{const mode=b.dataset.mode;if(mode==='free'){sequenceToken++;toast('Espelho Livre: ativando a câmera como sensor… ✨');startSensor();halo()}if(mode==='challenge')playSequence(['smile','blink','surprise','serious'],'Desafio Encantado começou! ⭐');if(mode==='princess')playSequence(['smile','surprise','blink','serious'],'Imite a Princesa 👑')}));
 ...
 window.addEventListener('tia:mirror-expression',e=>{const exp=e.detail?.expression;if(exp)setExpression(exp,false)});
 window.addEventListener('tia:mirror-toast',e=>{if(e.detail?.text)toast(e.detail.text)});
