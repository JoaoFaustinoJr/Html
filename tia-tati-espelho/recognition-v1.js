(()=>{
'use strict';
if(window.__TIA_TATI_FACE_RECOGNITION_V24__)return;
window.__TIA_TATI_FACE_RECOGNITION_V24__=true;
...
function classify(m){
 ...
 // Beijinho vem antes do sorriso: lábios projetados e boca pouco aberta.
 if(pucker>.40&&jaw<.34&&smile<.34)return 'kiss';
 // Piscada rápida: um olho bem fechado OU fechamento bilateral muito curto.
...
function loop(now){if(!running||!landmarker||!video)return;raf=requestAnimationFrame(loop);if(now-lastDetect<65||video.readyState<2||video.currentTime===lastVideoTime)return;lastDetect=now;lastVideoTime=video.currentTime;try{const result=landmarker.detectForVideo(video,now);stabilize(classify(toMap(result)),now)}catch(_){}}
async function start(){if(running){say('O sensor já está ativo. Toque novamente em Espelho Livre para parar.');return true}if(!navigator.mediaDevices?.getUserMedia){say('Este aparelho não oferece acesso à câmera pelo navegador.');return false}try{say('Preparando o sensor facial… ✨');state({status:'loading'});await createLandmarker();const v=makeVideo();stream=await navigator.mediaDevices.getUserMedia({audio:false,video:{facingMode:'user',width:{ideal:480},height:{ideal:640}}});v.srcObject=stream;await v.play();running=true;lastVideoTime=-1;lastEmitted='';candidate='';candidateSince=0;neutralSince=0;state({status:'active'});say('Sensor ativo! Sorria, pisque, faça espanto, sério ou mande um beijinho 💋');cancelAnimationFrame(raf);raf=requestAnimationFrame(loop);return true}catch(err){console.warn('[Espelho] sensor facial:',err);stop(false);state({status:'error',message:String(err?.message||err)});const denied=err?.name==='NotAllowedError'||err?.name==='PermissionDeniedError';say(denied?'Permita a câmera para o Espelho imitar você.':'Não consegui iniciar o sensor facial. Tente novamente.');return false}}
function stop(announce=true){running=false;cancelAnimationFrame(raf);raf=0;if(stream){stream.getTracks().forEach(t=>t.stop());stream=null}if(video)video.srcObject=null;candidate='';lastEmitted='';state({status:'off'});if(announce)say('Sensor facial pausado. Toque em Espelho Livre para iniciar novamente.')}
window.addEventListener('tia:mirror-start-sensor',()=>start());window.addEventListener('tia:mirror-stop-sensor',()=>stop());window.addEventListener('pagehide',()=>stop(false));document.addEventListener('visibilitychange',()=>{if(document.hidden&&running)stop(false)});window.TiaTatiFaceSensor={start,stop,get active(){return running}};
})();