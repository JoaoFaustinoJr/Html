(()=>{
'use strict';
if(window.__TIA_TATI_FACE_RECOGNITION_V26__)return;
window.__TIA_TATI_FACE_RECOGNITION_V26__=true;
...
function classify(m,now){
 ...
 const pucker=m.mouthPucker||0,funnel=m.mouthFunnel||0,mouthClose=m.mouthClose||0;
 const press=((m.mouthPressLeft||0)+(m.mouthPressRight||0))/2;
 const lipSignal=Math.max(pucker,funnel*.92,press*.62,mouthClose*.58);
 // Beijinho: a boca tem prioridade. Ao perceber início de biquinho, seguramos a piscada
 // por alguns quadros para não confundir o apertar natural dos olhos com uma piscada.
 const kissHint=lipSignal>.20&&jaw<.34&&smile<.34;
 if(kissHint){if(!kissIntentSince)kissIntentSince=now}else kissIntentSince=0;
 const kissReady=kissHint&&((pucker>.25)||(funnel>.29)||(lipSignal>.31))&&now-kissIntentSince>=45;
 if(kissReady)return 'kiss';
 const suppressBlink=kissHint&&now-kissIntentSince<280;
 if(!suppressBlink&&((maxBlink>.46&&minBlink<.60)||(blinkL>.60&&blinkR>.60)))return 'blink';
 if(jaw>.39&&(wide>.14||brow>.19))return 'surprise';
 if(smile>.33)return 'smile';
 if(smile<.18&&jaw<.20&&maxBlink<.38&&lipSignal<.20)return 'serious';
 return '';
}
function stabilize(exp,now){
 ...
 const hold=exp==='blink'?70:exp==='kiss'?90:exp==='serious'?430:170;
 if(now-candidateSince>=hold&&(exp!==lastEmitted||now-lastEmitAt>900)){lastEmitted=exp;emit(exp)}
}
function resetDetection(){candidate='';candidateSince=0;lastEmitted='';neutralSince=0;lastVideoTime=-1;lastDetect=0;kissIntentSince=0}
...
function pause(){if(!running)return;paused=true;candidate='';candidateSince=0;kissIntentSince=0;state({status:'active',paused:true})}
