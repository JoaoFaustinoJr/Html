...
function osc(freq,at,dur,type='sine',vol=.035,end=freq){
  const o=ctx.createOscillator(),g=env(at,dur,vol);
  o.type=type;
  o.frequency.setValueAtTime(freq,at);
  if(end!==freq)o.frequency.exponentialRampToValueAtTime(Math.max(25,end),at+dur);
  o.connect(g);o.start(at);o.stop(at+dur+.03);
}
function noise(at,dur=.05,vol=.025,highpass=5000){
  const len=Math.max(1,Math.floor(ctx.sampleRate*dur)),b=ctx.createBuffer(1,len,ctx.sampleRate),d=b.getChannelData(0);
  for(let i=0;i<len;i++)d[i]=(Math.random()*2-1)*(1-i/len);
  const src=ctx.createBufferSource(),f=ctx.createBiquadFilter(),g=env(at,dur,vol);
  f.type='highpass';f.frequency.value=highpass;src.buffer=b;src.connect(f);f.connect(g);src.start(at);
}
function kick(at){
  const o=ctx.createOscillator(),g=ctx.createGain();
  o.type='sine';o.frequency.setValueAtTime(118,at);o.frequency.exponentialRampToValueAtTime(43,at+.16);
  g.gain.setValueAtTime(.09,at);g.gain.exponentialRampToValueAtTime(.0001,at+.18);
...
function start(){
  ...
  started=true;step=0;nextStepTime=ctx.currentTime+.04;
...
function stop(){
  ...
  if(musicBus&&ctx)musicBus.gain.setTargetAtTime(.0001,ctx.currentTime,.06);
  updateBtn();
}
function setEnabled(v){
  enabled=!!v;localStorage.setItem(PREF,enabled?'1':'0');
...
function wrapVoice(){
  if(typeof window.playVoice==='function'&&!window.playVoice.__musicWrapped){
    ...
    const wrapped=function(...args){duck(3200);return original.apply(this,args);};
    wrapped.__musicWrapped=true;window.playVoice=wrapped;
  }
}
wrapVoice();
