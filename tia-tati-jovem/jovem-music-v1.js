...
function scheduler(){if(!ctx||!started)return;while(nextStepTime<ctx.currentTime+.12){scheduleStep(step,nextStepTime);step++;nextStepTime+=STEP;}const ducked=performance.now()<duckUntil;const target=ducked ? .055 : .20;musicBus.gain.cancelScheduledValues(ctx.currentTime);musicBus.gain.setTargetAtTime(enabled ? target : .0001,ctx.currentTime,.08);}
...
const btn=document.createElement('button');btn.type='button';btn.id='jovemMusicToggle';btn.setAttribute('aria-label','Ligar ou desligar música');btn.style.cssText='position:fixed;z-index:90000;right:12px;top:max(84px,calc(env(safe-area-inset-top) + 70px));width:44px;height:44px;border-radius:50%;border:1px solid rgba(69,232,255,.45);background:rgba(7,19,47,.82);backdrop-filter:blur(10px);box-shadow:0 8px 24px rgba(0,0,0,.28),0 0 18px rgba(54,220,255,.16);color:#fff;font:700 20px/1 system-ui;display:grid;place-items:center;cursor:pointer';
