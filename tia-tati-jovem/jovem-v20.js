(()=>{
'use strict';
if(window.__TIA_TATI_JOVEM_V20__)return;
window.__TIA_TATI_JOVEM_V20__=true;
...
function buildHero(){
  ...
  const section=q('.youth-challenges',home||document);
  if(!home||!section||q('.jovem-home-hero',home))return;
  const hero=document.createElement('section');
  hero.className='jovem-home-hero';
  hero.innerHTML=`<div class="jovem-home-copy"><div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center"><a class="jovem-home-back" href="../tia-tati/?v=20">← Portal Tia Tati</a><a class="jovem-home-back" href="manual-jovem.html?v=20">📘 Manual dos jogos</a></div><span class="jovem-home-kicker">TIA TATI • FISIO SENSORIAL</span><h1>Modo <em>Jovem</em></h1><p>No seu ritmo, você evolui. Desafios terapêuticos com uma linguagem visual mais madura, vibrante e direta.</p><div class="jovem-home-pills"><span>⚡ Resposta</span><span>🧠 Foco</span><span>🎵 Ritmo</span><span>🏁 Autonomia</span></div></div><div class="jovem-home-art" aria-hidden="true"><img src="assets/cards/reflexo-neon-v2.webp?v=20" alt=""><img src="assets/cards/ritmo-movimento-v2.webp?v=20" alt=""></div>`;
  home.insertBefore(hero,section);
}

function makeCard(kind,title,desc,art,badge){
  const b=document.createElement('button');
  b.className='youth-card '+kind;
  b.type='button';
  if(kind==='pulse-lab')b.dataset.youthSensory='pulse';
  if(kind==='recomeco-j4')b.dataset.youthBreathe='reset';
  if(kind==='movequest-j4')b.dataset.youthPhysical='quest';
  b.innerHTML=`<span class="youth-card-art ${kind==='movequest-j4'?'v41-move-art':''}">${kind==='movequest-j4'?'<b>🏁</b>':`<img src="${art}" alt="${title}" decoding="async">`}<em>${badge}</em></span><span class="youth-card-copy"><strong>${title}</strong><small>${desc}</small></span><span class="youth-card-go">Abrir</span>`;
  return b;
}
...
function tuneCards(){
  ...
  setText(q('.kicker',s),'DESAFIOS JOVENS');
  setHTML(q('.youth-challenges-head h2',s),'Escolha seu <span class="jovem-accent">desafio</span> ⚡');
  setText(q('.youth-challenges-head p',s),'Seis experiências para atenção, ritmo, memória, regulação e movimento.');
  setText(q('.youth-challenges-tag',s),'Jovens • adultos');
  const map=[
  ...
  map.forEach(([sel,name,desc])=>{
    ...
    setText(q('.youth-card-copy strong',c),name);
    setText(q('.youth-card-copy small',c),desc);
    setText(q('.youth-card-go',c),'Abrir');
  });
}

function hideSectionContaining(el){
  const sec=el?.closest('section');
  if(sec&&!sec.classList.contains('jovem-hide-shared-choice'))sec.classList.add('jovem-hide-shared-choice');
}
...
function isolateBreathe(){
  ...
  setText(q('.v41-head h2',s),'Recomeço');
  setText(q('.v41-head p',s),'Pausa guiada, foco e respiração em uma experiência visual jovem.');
  setText(q('.mission-badge',s),'⚡ RECOMEÇO');
  setHTML(q('.breathe-v41-hero h1',s),'Pausa. Foco.<br><em>Recomeço.</em>');
...
function isolateLight(){
  ...
  setText(q('.light-v18-head p, .light-head p',s),'Escolha a experiência jovem e avance no seu ritmo.');
}

function isolateHome(){
  const home=q('#screen-home');if(!home)return;
  home.classList.add('jovem-home-only');
  qa('.home-v22-hero,.home-v22-missions,.kids-v44,.audience-v44,.home-v22-autonomy,.support-v44,.home-v22-cards,.home-v22-tools,.finish-v44,.home-v22-footer-phrase,.home-v22-section-head',home).forEach(el=>el.classList.add('jovem-kids-hidden'));
  const nav=q('#bottomNav');if(nav)nav.classList.add('jovem-nav-hidden');
}

function apply(){
  document.documentElement.classList.add('tia-jovem-v20');
  isolateHome();
  ensureSixCards();
  buildHero();
  tuneCards();
