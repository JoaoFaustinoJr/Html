...
self.addEventListener('fetch',event=>{
  ...
  const live=/\/(app-v10\.js|comfort-v11\.js|v10\.css|comfort-v11\.css|manifest\.webmanifest)$/.test(u.pathname);
  if(live){event.respondWith(fetch(event.request,{cache:'no-store'}).then(resp=>{if(resp&&resp.ok){const clone=resp.clone();caches.open(CACHE).then(c=>c.put(event.request,clone))}return resp}).catch(()=>caches.match(event.request,{ignoreSearch:true})));return;}
