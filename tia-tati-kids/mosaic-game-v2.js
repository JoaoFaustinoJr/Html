(()=>{
'use strict';
if(window.__TIA_TATI_MOSAIC_FREE_V10__)return;
...
.tt-mosaic-wood-card .home-v22-visual{background:linear-gradient(145deg,#fff8fb,#eaf8ff);overflow:hidden}.tt-mosaic-wood-card .home-v22-visual img{width:100%;height:100%;object-fit:cover;object-position:center;display:block}
...
function pieceGeom(p){const s=slotById(p.sourceSlotId);return {w:p.w||s.w,h:p.h||s.h,clip:p.clip||s.clip}}
function cloneState(){return placed.map(p=>({id:p.id,type:p.type,sourceSlotId:p.sourceSlotId,x:p.x,y:p.y,w:p.w,h:p.h,clip:p.clip,rot:p.rot||0}))}
...
function unusedSources(type){return SLOTS.filter(s=>s.type===type&&!placed.some(p=>p.sourceSlotId===s.id))}
function nearestSource(type,x,y){const a=unusedSources(type);if(!a.length)return null;return a.reduce((best,s)=>{const cx=s.x+s.w/2,cy=s.y+s.h/2,d=Math.hypot(cx-x,cy-y);return !best||d<best.d?{s,d}:best},null).s}
...
function addCard(){const g=$('.home-v22-missions');if(!g)return false;let b=g.querySelector('[data-new-game="mosaic-wood"]');if(!b){b=document.createElement('button');g.appendChild(b)}b.type='button';b.className='home-v22-mission tt-new-game-card tt-mosaic-wood-card';b.dataset.newGame='mosaic-wood';b.innerHTML='<span class="home-v22-visual"><img src="assets/mosaico-card-v1.svg?v=1" alt="Mosaico da Tia Tati com peças coloridas de madeira"></span><span class="home-v22-mission-copy"><span class="home-v22-mission-icon">🧩</span><span><strong>Mosaico da Tia Tati</strong><small>Montar • explorar • criar padrões</small></span></span><em>Nova missão</em>';b.onclick=open;return true}

function makeUI(){
 if(overlay)return;
 overlay=document.createElement('div');overlay.className='ttmw';
 overlay.innerHTML='<div class="ttmw-shell"><div class="ttmw-top"><div class="ttmw-title"><img class="ttmw-avatar" src="assets/guide.webp" alt="Tia Tati"><div><strong>Mosaico da Tia Tati</strong><small>As mesmas peças podem formar novos desenhos</small></div></div><button class="ttmw-close">×</button></div><div class="ttmw-modes"><button class="ttmw-mode build active">🧩 Montar</button><button class="ttmw-mode explore">✨ Explorar</button><button class="ttmw-mode model">👁️ Modelo</button><button class="ttmw-mode sym">◐ Simetria</button></div><section class="ttmw-panel"><p class="ttmw-instruction">Arraste qualquer peça: a posição fica gravada diretamente na peça e não é recalculada ao soltar.</p><div class="ttmw-frame"><div class="ttmw-board"><div class="ttmw-axis"></div></div></div><div class="ttmw-palette-title">Peças disponíveis</div><div class="ttmw-palette"></div><div class="ttmw-actions"><button class="ttmw-view">👁️ Modelo</button><button class="ttmw-clear">↶ Desfazer</button><button class="ttmw-next">↻ Recomeçar</button></div><div class="ttmw-status">Monte o mosaico ou escolha Explorar.</div></section></div><div class="ttmw-modal"><div class="ttmw-modal-card"><h3>Modelo completo</h3><div class="ttmw-model-board"></div><button>Fechar modelo</button></div></div>';
...
function placeAt(e){
 ...
 const pt=boardPoint(e),source=nearestSource(selected,pt.x,pt.y);if(!source)return;
 pushHistory();
 let p={id:Date.now()+Math.random(),type:selected,sourceSlotId:source.id,x:source.x,y:source.y,w:source.w,h:source.h,clip:source.clip,rot:0};
 if(mode==='explore'){p.x=pt.x-p.w/2;p.y=pt.y-p.h/2;const q=clampFree(p,p.x,p.y,0);p.x=q.x;p.y=q.y}
 placed.push(p);renderPieces();refreshCounts();afterMove();
}
function renderPieces(){
 const board=$('.ttmw-board',overlay);$$('.ttmw-piece',board).forEach(e=>e.remove());
 placed.forEach(p=>{const el=document.createElement('div');el.className='ttmw-piece';el.dataset.id=p.id;el.style.left=p.x+'%';el.style.top=p.y+'%';el.style.width=p.w+'%';el.style.height=p.h+'%';el.style.clipPath=p.clip;el.style.backgroundColor=colorOf(p.type);el.style.transform='rotate('+(p.rot||0)+'deg)';el.addEventListener('pointerdown',ev=>startDrag(ev,p,el));el.addEventListener('dblclick',ev=>{ev.stopPropagation();rotatePiece(p)});board.appendChild(el)});
}
function startDrag(e,p,el){
 e.preventDefault();e.stopPropagation();dragBefore=cloneState();const pt=boardPoint(e);
 drag={p,el,dx:pt.x-p.x,dy:pt.y-p.y,pointerId:e.pointerId};try{el.setPointerCapture(e.pointerId)}catch(_){}
}
function moveDrag(e){
 if(!drag||e.pointerId!==drag.pointerId)return;e.preventDefault();const pt=boardPoint(e),q=clampFree(drag.p,pt.x-drag.dx,pt.y-drag.dy,drag.p.rot||0);drag.p.x=q.x;drag.p.y=q.y;drag.el.style.left=q.x+'%';drag.el.style.top=q.y+'%';
}
function finishDrag(e){
 if(!drag||e.pointerId!==drag.pointerId)return;e.preventDefault();
 const q=clampFree(drag.p,drag.p.x,drag.p.y,drag.p.rot||0);drag.p.x=q.x;drag.p.y=q.y;history.push(dragBefore);if(history.length>80)history.shift();
 try{drag.el.releasePointerCapture(e.pointerId)}catch(_){}
 renderPieces();drag=null;dragBefore=null;status('Peça ficou onde você soltou.');
}
...
function setMode(m){
 ...
 status(mode==='explore'?'Explore livremente: nenhuma troca de modo altera a posição das peças.':'Modo Montar: as peças novas entram na posição original; depois podem ser movidas livremente.');
}
