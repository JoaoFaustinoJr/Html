(()=>{
'use strict';
if(window.__TIA_TATI_KIDS_CARDS_UI_V7__)return;
window.__TIA_TATI_KIDS_CARDS_UI_V7__=true;
const CARDS=[
...
function fixHeaderImages(){qa('img[src*="assets/welcome.webp"]').forEach(img=>img.src='assets/cards/boas-vindas-v2.webp?v=7');}
function preload(){[...new Set(CARDS.map(c=>c.sprite))].forEach(src=>{const i=new Image();i.src=src+'?v=7';});}
function artMarkup(c){const shift=c.side?'-50%':'0%';return `<span class="tati-card-art"><img class="tati-sprite-img" src="${c.sprite}?v=7" alt="${c.title}" decoding="async" loading="eager" draggable="false" style="--tati-shift:${shift}"></span>`;}
