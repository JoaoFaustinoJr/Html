...
function ensure(){if(!enabled)return null;const AC=window.AudioContext||window.webkitAudioContext;if(!AC)return null;if(!ctx){ctx=new AC();master=ctx.createGain();bus=ctx.createGain();master.gain.value=.54;bus.gain.value=.15;bus.connect(master);master.connect(ctx.destination);}if(ctx.state==='suspended')ctx.resume().catch(()=>{});return ctx;}
...
function scheduler(){if(!ctx||!started)return;while(next<ctx.currentTime+.12){schedule(step,next);step++;next+=STEP;}const target=(performance.now()<duckUntil)?.038:.15;bus.gain.cancelScheduledValues(ctx.currentTime);bus.gain.setTargetAtTime(enabled?target:.0001,ctx.currentTime,.09);}
...
const btn=document.createElement('button');btn.type='button';btn.id='kidsMusicToggle';btn.setAttribute('aria-label','Ligar ou desligar música');btn.style.cssText='position:fixed;z-index:90000;right:14px;bottom:max(96px,calc(env(safe-area-inset-bottom) + 86px));width:44px;height:44px;border-radius:50%;border:1px solid rgba(242,71,154,.28);background:rgba(255,255,255,.92);box-shadow:0 8px 24px rgba(30,69,93,.18);color:#17457b;font:700 20px/1 system-ui;display:grid;place-items:center;cursor:pointer';
