(()=>{
'use strict';
if(window.__TIA_TATI_KIDS_CARDS_UI_V27__)return;
window.__TIA_TATI_KIDS_CARDS_UI_V27__=true;
const CARDS=[
...
function artMarkup(c){if(c.crop){const [x,y,w,h]=c.crop;return `<span class="tati-card-art"><span style="display:block;width:100%;height:100%;overflow:hidden;position:relative;background:#fff"><img class="tati-sprite-img" src="${c.image}?v=27" alt="${c.title}" decoding="async" loading="eager" draggable="false" style="width:100%;height:100%;object-fit:cover;object-position:center"></span></span>`;}if(c.image)return `<span class="tati-card-art${c.sheet?' tati-sheet-art':''}"><img class="tati-sprite-img" src="${c.image}?v=27" alt="${c.title}" decoding="async" loading="eager" draggable="false" style="width:100%;height:100%;object-fit:${c.sheet?'contain':'cover'};object-position:center"></span>`;const shift=c.side?'-50%':'0%';return `<span class="tati-card-art"><img class="tati-sprite-img" src="${c.sprite}?v=8" alt="${c.title}" decoding="async" loading="eager" draggable="false" style="--tati-shift:${shift}"></span>`;}
...
function buildSection(){
 ...
 grid.addEventListener('click',e=>{const p=e.target.closest('[data-preview]');if(p){const c=CARDS.find(x=>x.id===p.dataset.preview);if(c){if(window.TiaTatiCardExport)window.TiaTatiCardExport.previewSticker(c);else window.open((c.image||c.sprite)+'?v=27','_blank');}return;}const b=e.target.closest('[data-sticker]');if(!b)return;const c=CARDS.find(x=>x.id===b.dataset.sticker);if(c&&window.TiaTatiCardExport)window.TiaTatiCardExport.downloadSticker(c);else notice('Recurso preparando… tente novamente.');});
