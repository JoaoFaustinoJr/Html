(()=>{
'use strict';
if(window.__TIA_TATI_VOICE_EXPORT_V1__)return;
window.__TIA_TATI_VOICE_EXPORT_V1__=true;
...
function installUI(){
  ...
  panel.style.cssText='margin:14px 0 18px;padding:16px;border-radius:22px;background:linear-gradient(135deg,#fff6fb,#eefaff);border:1px solid rgba(43,91,112,.16);box-shadow:0 8px 24px rgba(24,66,91,.08);';
  panel.innerHTML=`
    <div style="display:flex;gap:12px;align-items:flex-start;justify-content:space-between;flex-wrap:wrap">
      <div style="flex:1;min-width:210px">
        <strong style="display:block;color:#173d71;font-size:1rem;margin-bottom:5px">Pacote definitivo de vozes</strong>
        <small style="display:block;color:#667f91;line-height:1.45">Depois de revisar as gravações, exporte tudo em um único arquivo. Esse pacote será usado para incorporar a voz da Tatiana ao app.</small>
      </div>
      <button id="exportVoicesBtn" type="button" style="border:0;border-radius:999px;padding:12px 17px;background:linear-gradient(90deg,#f2479a,#ff65ae);color:white;font-weight:900;box-shadow:0 8px 20px rgba(242,71,154,.2);cursor:pointer">⬇️ Exportar pacote de vozes</button>
    </div>`;
  (count?.parentElement||list?.parentElement||screen).insertBefore(panel,list||null);
